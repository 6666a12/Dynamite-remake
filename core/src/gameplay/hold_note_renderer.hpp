...
/**
 * Hold Note 渲染器 —— v1.1 规范 + 假身体/真身体设计
 *
 * 假身体 + 真身体设计（旧版保留）：
 *
 *   [head]      ← 在上方，先落
 *     |
 *   假身体      ← 半透明橙色 (head→判定线)，跟随 head 下落，长度可变
 *     |
 *   判定线
 *     |
 *   真身体      ← 正常颜色 (判定线→tail)，过线后从尾端逐渐收起
 *     |
 *   [tail]      ← 在下方，后落
 *
 * 渲染顺序：
 *   1. 假身体（从 head 外缘到判定线，半透明 + 纹理平铺）
 *   2. 真身体（从判定线到 tail，正常颜色 + 纹理平铺，过线后收起）
 *   3. 边框（整个假身体 + 真身体区域）
 *   4. 进度高亮（按住时从判定线端填充）
 *   5. Hold 头部（覆盖在假身体起点上）
 */
